...
#define PROGRAM_FILE "conv.cl"
// name of the clkernel function
#define KERNEL_FUNC "Conv"
...
cl_device_id create_device() {
   ...
   if(clerr < 0) {
    ...
    cl_uint addr_bits;
    clGetDeviceInfo(dev, CL_DEVICE_ADDRESS_BITS, sizeof(addr_bits), &addr_bits, NULL);
    printf("  CL_DEVICE_ADDRESS_BITS:\t\t%u\n", addr_bits);

		// CL_DEVICE_MAX_CLOCK_FREQUENCY
    cl_uint clock_frequency;
    clGetDeviceInfo(dev, CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(clock_frequency), &clock_frequency, NULL);
    printf("  CL_DEVICE_MAX_CLOCK_FREQUENCY:\t%u MHz\n", clock_frequency);

		// CL_DEVICE_MAX_WORK_ITEM_SIZES
    size_t workitem_size[3];
    clGetDeviceInfo(dev, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(workitem_size), &workitem_size, NULL);
    printf("  CL_DEVICE_MAX_WORK_ITEM_SIZES:\t%u / %u / %u \n", workitem_size[0], workitem_size[1], workitem_size[2]);

    // CL_DEVICE_MAX_WORK_GROUP_SIZE
    size_t workgroup_size;
    clGetDeviceInfo(dev, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(workgroup_size), &workgroup_size, NULL);
    printf("  CL_DEVICE_MAX_WORK_GROUP_SIZE:\t%u\n", workgroup_size);


		// CL_DEVICE_LOCAL_MEM_SIZE
		cl_ulong mem_size;
    clGetDeviceInfo(dev, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(mem_size), &mem_size, NULL);
    printf("  CL_DEVICE_LOCAL_MEM_SIZE:\t\t%u KByte\n", (unsigned int)(mem_size / 1024));

		// CL_DEVICE_LOCAL_MEM_TYPE
    cl_device_local_mem_type local_mem_type;
    clGetDeviceInfo(dev, CL_DEVICE_LOCAL_MEM_TYPE, sizeof(local_mem_type), &local_mem_type, NULL);
    printf("  CL_DEVICE_LOCAL_MEM_TYPE:\t\t%s\n", local_mem_type == 1 ? "local" : "global");

		// CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE
    clGetDeviceInfo(dev, CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE, sizeof(mem_size), &mem_size, NULL);
    printf("  CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE:\t%u Byte\n", (unsigned int)(mem_size));

    //CL_DEVICE_GLOBAL_MEM_CACHE_SIZE
		cl_ulong g_cache_size;
    clGetDeviceInfo(dev, CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, sizeof(g_cache_size), &g_cache_size, NULL);
    printf("  CL_DEVICE_GLOBAL_MEM_CACHE_SIZE:\t\t%u Byte\n", (unsigned int)(g_cache_size));

		//CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE
		cl_uint cache_line_size;
    clGetDeviceInfo(dev, CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE:\t\t%u \n", (unsigned int)(cache_line_size));

		//CL_DEVICE_MEM_BASE_ADDR_ALIGN
    clGetDeviceInfo(dev, CL_DEVICE_MEM_BASE_ADDR_ALIGN, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_MEM_BASE_ADDR_ALIGN:\t\t%u \n", (unsigned int)(cache_line_size));

		//CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE
    clGetDeviceInfo(dev, CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE:\t\t%u \n", (unsigned int)(cache_line_size));

		//CL_DEVICE_NATIVE_VECTOR_WIDTH_FLOAT
		clGetDeviceInfo(dev, CL_DEVICE_NATIVE_VECTOR_WIDTH_FLOAT, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_NATIVE_VECTOR_WIDTH_FLOAT:\t\t%u \n", (unsigned int)(cache_line_size));
		//CL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE
		clGetDeviceInfo(dev, CL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE:\t\t%u \n", (unsigned int)(cache_line_size));
		//CL_DEVICE_NATIVE_VECTOR_WIDTH_INT
		clGetDeviceInfo(dev, CL_DEVICE_NATIVE_VECTOR_WIDTH_INT, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_NATIVE_VECTOR_WIDTH_INT:\t\t%u \n", (unsigned int)(cache_line_size));
		//CL_DEVICE_NATIVE_VECTOR_WIDTH_CHAR
		clGetDeviceInfo(dev, CL_DEVICE_NATIVE_VECTOR_WIDTH_CHAR, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_NATIVE_VECTOR_WIDTH_CHAR:\t\t%u \n", (unsigned int)(cache_line_size));

		//CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT
		clGetDeviceInfo(dev, CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT:\t\t%u \n", (unsigned int)(cache_line_size));
		//CL_DEVICE_PREFERRED_VECTOR_WIDTH_DOUBLE
		clGetDeviceInfo(dev, CL_DEVICE_PREFERRED_VECTOR_WIDTH_DOUBLE, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_PREFERRED_VECTOR_WIDTH_DOUBLE:\t\t%u \n", (unsigned int)(cache_line_size));
		//CL_DEVICE_PREFERRED_VECTOR_WIDTH_INT
		clGetDeviceInfo(dev, CL_DEVICE_PREFERRED_VECTOR_WIDTH_INT, sizeof(cache_line_size), &cache_line_size, NULL);
    printf("  CL_DEVICE_PREFERRED_VECTOR_WIDTH_INT:\t\t%u \n", (unsigned int)(cache_line_size));
		//CL_DEVICE_PREFERRED_VECTOR_WIDTH_CHAR
		clGetDeviceInfo(dev, CL_DEVICE_PREFERRED_VECTOR_WIDTH_CHAR, sizeof(cache_line_size), &cache_line_size, NULL);
...
cl_program clprogram[3];
cl_kernel clkernel[3];
...
void initOpenCL(){
	...
	// Create a clkernel(mul kernel)
	// creates a clprogram and build it
	clprogram[2] = build_program(clcontext, cldevice, "kernels_sgemm.cl");
	clkernel[2] = clCreateKernel(clprogram[2], "myGEMM", &clerr);
	if(clerr < 0) {
		 perror("Couldn't create a clkernel[2] ^^^^");
		 //LOG(INFO) << "the error num is clerr = " << clerr;
		 exit(1);
	}
}
                              ...
                              transa = 1;
														    ...
														    if(clerr < 0) {
														       ...
														       exit(1);
																...
																int ta=0;
																if (transa == true) ta=1;
																clerr = clSetKernelArg(clkernel[2], 0, sizeof(cl_int),(void*) &M);
														    clerr |= clSetKernelArg(clkernel[2], 1, sizeof(cl_int),(void*) &N);
														    clerr |= clSetKernelArg(clkernel[2], 2, sizeof(cl_int),(void*) &K);
																//clerr |= clSetKernelArg(clkernel[2], 3, sizeof(cl_mem), &d_m1);
																clerr |= clSetKernelArg(clkernel[2], 3, sizeof(cl_mem), &a_trans);
														    clerr |= clSetKernelArg(clkernel[2], 4, sizeof(cl_mem), &d_m2);
																clerr |= clSetKernelArg(clkernel[2], 5, sizeof(cl_mem), &d_res);
																clerr |= clSetKernelArg(clkernel[2], 6, sizeof(cl_int),(void*) &ta);


																clerr |= clSetKernelArg(clkernel[2], 7, sizeof(cl_float)*k, NULL);


																if(clerr < 0) {
														       perror("Couldn't create a clkernel[2] argument");
														       exit(1);
														    }

																double start = timing();
																// Enqueue the created clkernel
														    clerr = clEnqueueNDRangeKernel(clqueue, clkernel[2], 1, NULL, &global_size,
...
class ConvolutionOp : public Operator {
 public:
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    //LOG(INFO) << "data_shape: " << data.shape_;
	  //LOG(INFO) << "wmat_shape: " << wmat_shape;
    ...
    for (index_t i = 0; i < nbatch; i += nstep_) {
      ...
      for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
        ...
        temp_dst[gid] = dot(wmat[gid], tmpc);
				...
				#ifdef CONV1
				int w_0,w_1,d_0;
				w_0 = wmat[gid].shape_.shape_[0];
				w_1 = wmat[gid].shape_.shape_[1];
				d_0 = tmpc.shape_.shape_[1];
				float *wmatPtr,*dataPtr,*resPtr;
				wmatPtr = wmat[gid].dptr_;
				dataPtr = tmpc.dptr_;
				resPtr = temp_dst[gid].dptr_;
				float ACC=0.0;
				for(int i=0;i<w_0;i++)
					for(int j=0;j<d_0;j++)
					{
						ACC = 0.0;
						for(int k=0;k<w_1;k++)
							ACC += wmatPtr[i*w_1+k]*dataPtr[k*d_0+j];
				    resPtr[i*d_0+j] = ACC;
					}
				//LOG(INFO) << "(w0,w1)= "<< "(" << w_0 << "," << w_1 << ")";
				//LOG(INFO) << "d_0= "<< d_0;
				#endif
        ...
        LOG(INFO) << "(kernel_stride0,kernel_stride1)= "<< "(" << kernel_stride0 << "," << kernel_stride1 << ")";
            ...
            dataP = dataPtr;
					...
					d_2 += 2*param_.pad[1];
          float *dataPPtr;
          dataPPtr = (float *)calloc(d_0*d_1*d_2,sizeof(float));
					//LOG(INFO) << "dataPPtr:" << dataPPtr[0] << " " << dataPPtr[1];

					for(int ii=0;ii<d_0;ii++)
					  for(int jj=param_.pad[0];jj<d_1-param_.pad[0];jj++)
						  for(int kk=param_.pad[1];kk<d_2-param_.pad[1];kk++)
							  //dataPPtr[ii][jj][kk] = dataPtr[ii][jj-param_.pad[0]][kk-param_.pad[1]];
								dataPPtr[ii*d_1*d_2+jj*d_2+kk] = dataPtr[ii*(d_1-2*param_.pad[0])*(d_2-2*param_.pad[1])+(jj-param_.pad[0])*(d_2-2*param_.pad[1])+kk-param_.pad[1]];
          dataP = dataPPtr;
					/*for(int ii=0;ii<out_0;ii++)
						for(int jj=0;jj<out_1;jj++)
							for(int kk=0;kk<out_2;kk++) {
								float tempAcc = 0.0;
								for(int mm=0;mm<dd_0;mm++)
									for(int pp=0;pp<kernel_0;pp++)
										for(int tt=0;tt<kernel_1;tt++)
											//resPtr[ii][jj][kk]+=wmatPtr[ii][mm][pp][tt]*dataPtr[mm][kernel_stride0*jj+pp][kernel_stride1*kk+tt];
											tempAcc += wmatPtr[ii*dd_0*kernel_0*kernel_1+mm*kernel_0*kernel_1+pp*kernel_1+tt]* \
											dataP[mm*dd_1*dd_2+(kernel_stride0*jj+pp)*dd_2+kernel_stride1*kk+tt];
								resPtr[ii*out_1*out_2+jj*out_2+kk] = tempAcc;
							}
        ...
        //////////////////////////////////////////////////OpenCL Implementation//////////////////////////////////////////////
				LOG(INFO) << "================= Convolution OpenCL Implementation=======================";
        ...
        //double start = timing();
				...
				// create the data buffers to be sent to devices
				d_m1 = clCreateBuffer(clcontext, CL_MEM_READ_ONLY |
							CL_MEM_USE_HOST_PTR, wmatSize, wmatPtr, &clerr);
				d_m2 = clCreateBuffer(clcontext, CL_MEM_READ_ONLY |
							CL_MEM_USE_HOST_PTR, dataSize, dataP, &clerr);
				d_res = clCreateBuffer(clcontext, CL_MEM_READ_WRITE |
							CL_MEM_USE_HOST_PTR, resSize, resPtr, &clerr);
				if(clerr < 0) {
					 perror("Couldn't create a buffer");
					 exit(1);
				}
        ...
        LOG(INFO) << "Transfer data time: " << end1- start1 ;
				...
				clerr |= clSetKernelArg(clkernel[0], 1, sizeof(cl_int),(void*) &out_1);
				clerr |= clSetKernelArg(clkernel[0], 2, sizeof(cl_int),(void*) &out_2);
				clerr |= clSetKernelArg(clkernel[0], 3, sizeof(cl_int),(void*) &d_0);
				clerr |= clSetKernelArg(clkernel[0], 4, sizeof(cl_int),(void*) &d_1);
				clerr |= clSetKernelArg(clkernel[0], 5, sizeof(cl_int),(void*) &d_2);
				clerr |= clSetKernelArg(clkernel[0], 6, sizeof(cl_int),(void*) &kernel_0);
				clerr |= clSetKernelArg(clkernel[0], 7, sizeof(cl_int),(void*) &kernel_1);
				clerr |= clSetKernelArg(clkernel[0], 8, sizeof(cl_int),(void*) &kernel_stride0);
				clerr |= clSetKernelArg(clkernel[0], 9, sizeof(cl_int),(void*) &kernel_stride1);

			  clerr |= clSetKernelArg(clkernel[0], 10, sizeof(cl_mem), &d_m1);
				//clerr |= clSetKernelArg(clkernel[0], 3, sizeof(cl_mem), &a_trans);
				clerr |= clSetKernelArg(clkernel[0], 11, sizeof(cl_mem), &d_m2);
				...
				*/
				...
				double start = timing();
				// Enqueue the created clkernel
				clerr = clEnqueueNDRangeKernel(clqueue, clkernel[0], 2, NULL, global_size,
							local_size, 0, NULL, NULL);
				if(clerr < 0) {
					 perror("Couldn't enqueue the clkernel[1] PORRA");
					 LOG(INFO) << "the error num is clerr = " << clerr;
				...
				const size_t global_size = out_0*out_1*out_2;
        ...
        double end = timing();
				...
				#endif
				/*LOG(INFO) << "wmat[gid] shape:" << wmat[gid].shape_;
				LOG(INFO) << "tmpc shape:" << tmpc.shape_;
				LOG(INFO) << "temp_dst[gid] shape:" << temp_dst[gid].shape_;
				LOG(INFO) << "data shape:" << data.shape_;
				LOG(INFO) << "pad[0] [1]:" << param_.pad[0] << param_.pad[1];
				LOG(INFO) << "stride in data is :" << data.stride_;
				LOG(INFO) << "stride in wmat[gid] is :" << wmat[gid].stride_;
				LOG(INFO) << "step :" << step;*/
      }
      //out.Slice(i, i + step) = swapaxis<1, 0>(reshape(temp_dst,
      //                                        mshadow::Shape4(param_.num_filter,
...
enum ElementWiseBinaryOpType {kPlus, kMinus, kMul, kDiv, kPower};
}  // elembinary
